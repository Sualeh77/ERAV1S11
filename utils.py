...
def image_denormalizer(image, std_, mean_):
    """
    ...
    """
    return image.permute(1,2,0) * np.array(std_) + np.array(mean_)

#########################################################################################################################################################

def get_GradCAM_heatmap(model, target_layers, use_cuda:bool, target, img:np.array, transparency):
    """
    ...
    grayscale_cam = cam(input_tensor=img.permute(2,0,1).unsqueeze(dim=0).to(torch.float32), targets=targets)
